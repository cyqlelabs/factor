...
Then act before you report. Read the failures for what they say first: a call refused for its arguments — a missing field, a key or path that does not exist, a query the service rejected — is the caller's mistake, not the tool's, and is evidence of nothing; a call of your own in this check counts the same. A feature that has failed every time it ran on this machine is broken, not transient: switch it off with config_set (read the key with config_get first) or repair what it needs, rather than judging it again next time. Never disable a tool from here — which tools exist is the user's decision, so name the tool and the cause and let them decide. A version that is behind is upgraded with the upgrade tool; a missing helper is installed with pkg_install. Never kill or restart processes with exec — the sidecars are supervised, and one you start by hand outlives your turn as an orphan. What you could not fix, or what only the user can decide, goes to them in a line or two with the cause named. Do not write HEARTBEAT_OK in a reply that reports a change or a finding, since it silences the whole reply; HEARTBEAT_OK alone is for when nothing was worth changing and nothing needs the user.
`
