...
func newTestBridge(t *testing.T, run TurnFunc) *testBridge {
	...
	if run == nil {
		run = func(_ context.Context, content, sessionKey, _, _ string, _ func(string)) (string, error) {
			tb.turns <- content
...
func TestBridgeCancellationReachesTheTurn(t *testing.T) {
	...
	tb := newTestBridge(t, func(ctx context.Context, _, _, _, _ string, _ func(string)) (string, error) {
		close(started)
		<-ctx.Done()
		cancelled <- ctx.Err()
		return "", ctx.Err()
...
func TestBridgeRefusesCallersOutsideTheAllowlist(t *testing.T) {
	...
	tb.bindRunner(func(_ context.Context, _, sessionKey, _, _ string, _ func(string)) (string, error) {
		tb.sessions <- sessionKey
...
func TestBridgeSpeaksAnApologyWhenATurnFails(t *testing.T) {
	tb := newTestBridge(t, func(context.Context, string, string, string, string, func(string)) (string, error) {
...
func TestBridgeNeverSpeaksAnEmptyReply(t *testing.T) {
	tb := newTestBridge(t, func(context.Context, string, string, string, string, func(string)) (string, error) { return "  ", nil })
	resp := tb.post(t, "/v1/chat/completions", chatBody("hello", false),
		map[string]string{"X-Factor-Caller": "+15550001111"})
	var parsed chatResponse
	if err := json.NewDecoder(resp.Body).Decode(&parsed); err != nil {
...
func TestBridgeStreamsAMidTurnNoteBeforeTheAnswer(t *testing.T) {
	...
	tb := newTestBridge(t, func(_ context.Context, _, _, _, _ string, notice func(string)) (string, error) {
