...
func TestParallelSafeBatchRunsConcurrently(t *testing.T) {
	...
	go func() { done <- h.loop.runTools(context.Background(), "cli:x", nil, calls, false) }()
...
func TestOneUnsafeCallMakesTheBatchSequential(t *testing.T) {
	...
	h.loop.runTools(context.Background(), "cli:x", nil, calls, false)
...
func TestSingleCallIsNeverParallel(t *testing.T) {
	...
	h.loop.runTools(context.Background(), "cli:x", nil, callsTo("reader", "1"), false)
...
func TestRunToolsAnswersRepeatedCallsOnce(t *testing.T) {
	...
	outcomes := h.loop.runTools(context.Background(), "cli:x", nil, calls, false)
...
func TestCancelledBatchReportsInterruptionNotFailure(t *testing.T) {
	...
	cancel()
	outcomes := h.loop.runTools(ctx, "cli:x", nil, callsTo("reader", "1", "2"), false)
