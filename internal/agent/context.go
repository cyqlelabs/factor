...
const toolDiscipline = "Still in force this turn, from the rules at the top of this prompt: " +
	...
	"when the user says something had no effect, look from the receiving side before changing the sender; " +
...
- When a task gathers results across many tool calls, write each one to a file as it arrives and build the answer from the files: old tool results are cleared from your context to make room, and fetching a page again only to lose it again is a loop.
